...
:setupNT
\maintenance\boot\syslinux.exe -maf -d /maintenance/boot/ %DISK%:
goto setupEFI

:setup95
\maintenance\boot\syslinux.com -maf -d /maintenance/boot/ %DISK%:
goto setupEFI

:setupEFI
mkdir %DISK%:\EFI\Boot
copy \maintenance\boot\EFI\Boot\* %DISK%:\EFI\Boot
goto setupDone
